...
Module.register("MMM-DagelijksWoord", {
  ...
  getDom: function () {
    ...
    wrapperVerse.classList.add("dagelijkswoord-verse");
    labelSource.innerHTML = this.source;
    labelSource.classList.add("dagelijkswoord-source");
    ...
    if (this.donationDay()) {
      // only on donation request day (monday after christmas)
      var wrapperDonation = document.createElement("div");
      wrapperDonation.innerHTML = this.donationText;
      wrapperDonation.classList.add("dagelijkswoord-donation");
  ...
  donationDay: function () {
    const date = new Date();
    if (date.getMonth() === 11 && date.getDate() === 27 && date.getDay !== 0)
      return true; //day after chrismass and not on a sunday.
    else if (
      date.getMonth() === 11 &&
      date.getDate() === 28 &&
      date.getDay === 1
    )
      return true; //day after chrismass when second chrismass day is a sunday.
    else return false;
  }
});
